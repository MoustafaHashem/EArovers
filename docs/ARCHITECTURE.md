...
- **`RoleHistory`**: Maps a `Member` to specific clan roles in specific academic years, establishing the clan hierarchy and tree. (Note: The `tier` column was deprecated; we now rely on heuristic tier mapping via `roleTitle` and a `ParticipantStatus` enum).
